<?php

namespace App\Http\Controllers\Admin;

use App\Events\Gallery\GalleryChanged;
use App\Http\Requests\Admin\GalleryRequest;
use App\Models\Gallery;
use App\Repositories\Interfaces\GalleryRepositoryInterface;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\View;
use Illuminate\Support\Str;

class GalleryController extends BaseController
{
    protected $module;

    protected $model;

    protected $nameItem;

    protected $imageFolder;

    protected $galleryRepository;

    public function __construct(GalleryRepositoryInterface $galleryRepository, $imageFolder = 'gallery')
    {
        $this->module = 'gallery';
        $this->model = new Gallery;
        $this->nameItem = 'Hình ảnh';
        $this->imageFolder = $imageFolder;
        $this->galleryRepository = $galleryRepository;

        parent::__construct($this->module, $imageFolder);

        View::share('nameClass', $imageFolder);
    }

    public function index(Request $request)
    {
        $filters = [
            'search' => $request->input('search'),
            'type' => $request->input('type', ''),
            'sort_field' => 'stt',
            'sort_direction' => 'asc'
        ];

        $data['list'] = $this->galleryRepository->getFilteredGalleries($filters);
        $data['nameItem'] = $this->nameItem;

        return $this->view_admin('list', $data);
    }

    public function create()
    {
        $data['action'] = 'create';
        $data['nameItem'] = $this->nameItem;

        return $this->view_admin('detail', $data);
    }

    public function store(GalleryRequest $request)
    {
        $data = $request->except('_token', 'return_back', 'return_list');

        // Handle image - Save new image
        $data['image'] = $this->saveImageWithWatermark($request);

        $gallery = $this->galleryRepository->create($data);
        toast('Thêm '.$this->nameItem.' thành công', 'success');

        GalleryChanged::dispatch($gallery, 'created');

        return $request->has('return_back') ? back() : ($request->has('return_list') ? $this->route_admin('index') : null);
    }

    public function edit($uuid, $currentPage)
    {
        $gallery = $this->galleryRepository->findByUuid($uuid);

        if (! $gallery) {
            toast('Không tìm thấy '.$this->nameItem, 'error');

            return back();
        }

        $data = [
            'page' => $gallery,
            'action' => 'edit',
            'nameItem' => $this->nameItem,
            'currentPage' => $currentPage,
            'imageFolder' => $this->imageFolder,
        ];

        return $this->view_admin('detail', $data);
    }

    public function update(GalleryRequest $request, string $uuid)
    {
        $current = $this->galleryRepository->findByUuid($uuid);
        $data = $request->except('_token', 'return_back', 'return_list', 'currentPage');

        // Handle image - Update existing image
        $data['image'] = $this->updateImageWithWatermark($request, $current);

        $this->galleryRepository->update($data, $uuid);
        toast('Cập nhật '.$this->nameItem.' thành công', 'success');

        GalleryChanged::dispatch($current, 'updated');

        return $this->route_admin('index', [], [], $request->input('currentPage'));
    }

    public function status($uuid, $status, $field)
    {
        $gallery = $this->galleryRepository->findByUuid($uuid);
        $result = $this->toggleService->toggleModelStatus($uuid, $status, $field, $this->model::class);

        // Remove related cache
        GalleryChanged::dispatch($gallery, 'status_updated');

        return $result;
    }

    public function destroy(string $uuid)
    {
        $gallery = $this->galleryRepository->findByUuid($uuid);

        if (! $gallery) {
            toast('Không tìm thấy '.$this->nameItem, 'error');

            return back();
        }
        GalleryChanged::dispatch($gallery, 'destroy');

        // Gọi destroyData để xóa cả hình ảnh (event sẽ được dispatch tự động)
        return $this->dataRemovalService->destroyData($this->model::class, $uuid, $this->imageFolder);
    }

    public function destroyAll(Request $request)
    {
        $uuids = $request->input('uuids', []);

        if (empty($uuids)) {
            toast('Không có mục nào được chọn để xóa.', 'error');

            return redirect()->back();
        }

        // Lấy thông tin galleries trước khi xóa
        $galleries = $this->galleryRepository->findByUuids($uuids);

        GalleryChanged::dispatch(null, 'deleted');

        // Gọi destroyAllByUUIDs để xóa cả hình ảnh (event sẽ được dispatch tự động)
        return $this->dataRemovalService->destroyAllByUUIDs($this->model::class, $uuids, $this->imageFolder);
    }

    public function numericalOrder(Request $request, $uuid)
    {
        $gallery = $this->galleryRepository->findByUuid($uuid);
        $result = $this->toggleService->updateModelOrder($request, $uuid, $this->model::class);

        // Remove related cache
        GalleryChanged::dispatch($gallery, 'order_updated');

        return $result;
    }
}
