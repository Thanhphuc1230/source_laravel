...
<body>
  <nav class="navbar sticky-top navbar-expand-lg navbar-dark" id="nav">
    ...
    <div class="collapse navbar-collapse flex-grow-0" id="nav-buttons">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link" data-display="grid" href="{{ route('admin.analytics.index') }}">
            <span>← Trở lại Dashboard</span>
          </a>
        </li>
        <li class="nav-item">
  ...
  <div class="d-flex flex-row">
    ...
    <div id="main">
      ...
      <div class="lfm-search-bar" style="margin-bottom: 15px;">
        <input type="text" id="lfm-search-input" class="form-control" placeholder="Tìm kiếm file...">
      </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
        const searchInput = document.getElementById('lfm-search-input');
        ...
        // Gọi filterFiles khi input thay đổi
        searchInput.addEventListener('input', filterFiles);

        // Gọi filterFiles mỗi khi nội dung #content thay đổi (có delay nhỏ)
        const observer = new MutationObserver(function() {
            setTimeout(filterFiles, 50); // delay 50ms để đảm bảo DOM đã render xong
        });
        observer.observe(document.getElementById('content'), { childList: true, subtree: false });

        // Khi trang vừa load, có thể nội dung chưa sẵn sàng, nên delay filterFiles
        setTimeout(filterFiles, 200);
    });
  </script>
</body>
</html>
