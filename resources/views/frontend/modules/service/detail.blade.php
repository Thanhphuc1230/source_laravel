...
@section('module', lang($service_detail, 'name'))
@section('keywords', lang($service_detail, 'keyword'))
@section('description', lang($service_detail, 'description'))
@section('images', lang($service_detail, 'image') ?? $web->logo)
...
@section('content')
    <div class="bg-gray-100 py-4 border-b border-gray-200/50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-2xs text-gray-500 flex items-center space-x-2">
            ...
            @if($service_detail->cate)
                <a href="{{ route('web.resolve', ['slug' => $service_detail->cate->slug]) }}" class="hover:text-emerald-950">{{ lang($service_detail->cate, 'name') }}</a>
                <i class="fa-solid fa-chevron-right text-3xs"></i>
            @endif
            <span class="text-gray-700 font-semibold truncate">{{ lang($service_detail, 'name') }}</span>
        </div>
    </div>
                ...
                @if(isset($related_services) && $related_services->isNotEmpty())
                    <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-4">
                        <h3 class="font-heading font-bold text-sm text-emerald-950 border-b border-gray-100 pb-3 flex items-center">
                            <i class="fa-solid fa-concierge-bell text-emerald-800 mr-2"></i>
